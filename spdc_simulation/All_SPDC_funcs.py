...
def Gaussian_beam_calc_angle(beam,crystal,z_point, angledeg, shiftX, zShift): 
    X, Y = np.meshgrid(crystal.x,crystal.y, indexing='ij')
    xi   = 2*(z_point-zShift)/beam.b
    tau  = 1/(1+1j*xi) 
    kz=beam.k*np.sqrt(1/(1+(np.tan(angledeg*0.017)**2)))
    kx=np.tan(0.017*angledeg)*kz
    E    = E0(beam.power, beam.n, beam.waist)*tau*np.exp(-((X-shiftX)**2+(Y)**2)/(beam.waist**2)*tau)*np.exp(1j*kz*(z_point-zShift))*np.exp(1j*kx*(X-shiftX))
    return E
...
def crystal_prop_indistuigishable(Pump,Siganl_field,crystal):
    ...
    for z in crystal.z: 
        #pump beam:
#        E_pump   = Gaussian_beam_calc_angle(Pump,crystal,z, 2, 237e-6) + np.exp(1j*pi)*Gaussian_beam_calc_angle(Pump,crystal,z, -2, -237e-6) 
        E_pump   = Gaussian_beam_calc_angle(Pump,crystal,z, 0,0) 
        #crystal slab:
        PP           = crystal.slab(crystal.poling_period,z)
